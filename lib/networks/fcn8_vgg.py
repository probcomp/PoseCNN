...
class fcn8_vgg:
    def __init__(self, num_classes, vgg16_npy_path=None):
        ...
        self.gt_label_2d = tf.placeholder(tf.float32, shape=[None, None, None, num_classes])

        # define a queue
        q = tf.FIFOQueue(100, [tf.float32, tf.float32])
        self.enqueue_op = q.enqueue([self.data, self.gt_label_2d])
        self.data_queue, self.gt_label_2d_queue = q.dequeue()
        self.close_queue_op = q.close(cancel_pending_enqueues=True)

        self.build(self.data_queue, train=True, num_classes=num_classes)
    ...
    def build(self, bgr, train=False, num_classes=20, random_init_fc8=False,
        ...
        self.prob = self._log_softmax_high_dimension(self.upscore32, num_classes, name='prob')

        self.label_2d = tf.argmax(self.prob, dimension=3)

    def _log_softmax_high_dimension(self, input, num_classes, name):
