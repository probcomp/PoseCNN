...
class yumi(datasets.imdb):
    def __init__(self, image_set, yumi_path = None):
        datasets.imdb.__init__(self, 'yumi_' + image_set)
        self._image_set = image_set
        self._yumi_path = self._get_default_path() if yumi_path is None \
                            else yumi_path
        ...
        assert os.path.exists(self._yumi_path), \
                'yumi path does not exist: {}'.format(self._yumi_path)
    ...
    def _load_image_set_index(self):
        ...
        """
        image_set_file = os.path.join(self._yumi_path, self._image_set + '.txt')
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(datasets.ROOT_DIR, 'data', 'Yumi')
    ...
    def _load_object_points(self):
        ...
        for i in range(1, len(self._classes)):
            point_file = os.path.join(self._yumi_path, 'models', self._classes[i] + '.xyz')
            print(point_file)
    ...
    def _load_object_extents(self):
        ...
        extent_file = os.path.join(self._yumi_path, 'extents.txt')
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_yumi_annotation(index)
    ...
    def _load_yumi_annotation(self, index):
        """
